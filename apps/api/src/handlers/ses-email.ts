...
const TABLE = process.env.EMAIL_CAMPAIGNS_TABLE ?? `blossompot-email-campaigns-${process.env.ENVIRONMENT ?? "dev"}`;
const ORDERS_TABLE = process.env.ORDERS_TABLE ?? `blossompot-orders-${process.env.ENVIRONMENT ?? "dev"}`;
...
function defaultSettings(): SesSettings {
  ...
  return sesSettingsSchema.parse({
    ...
    defaultSenderName: "BlossomPot",
    // Mailercloud verified Sender ID (From). SMTP login user may differ (smtpUser).
    defaultSenderEmail:
      process.env.MARKETING_FROM_EMAIL || process.env.SES_FROM_EMAIL || "email@blossompot.com",
    defaultReplyTo:
      process.env.MARKETING_FROM_EMAIL || process.env.SES_REPLY_TO || "email@blossompot.com",
    dailyLimit: 50_000,
    ...
    smtpUser: process.env.MARKETING_SMTP_USER || "order@blossompot.com",
...
export async function updateSettings(event: APIGatewayProxyEventV2) {
  ...
  if (/^(smtp|mail)\.blossompot\.com$/.test(host)) {
    return badRequest(
      "Marketing SMTP must use Mailercloud (smtp-prod.mailrcld.com). smtp.blossompot.com is reserved for transactional order emails only."
    );
  }
  ...
  const settingsToSave = {
    ...
    smtpUser: parsed.data.smtpUser?.trim() || "order@blossompot.com",
...
export async function sendTest(event: APIGatewayProxyEventV2) {
  ...
  try {
    ...
    const settings = await loadSettings();
    const fromName = (campaign.senderName || settings.defaultSenderName || "BlossomPot").trim();
    // From must be Mailercloud verified Sender ID (email@blossompot.com), not SMTP login.
    const fromEmail = (
    ...
    if (!fromEmail) {
      return badRequest(
        "Sender email is missing. Set Default sender email to your verified Mailercloud Sender ID (e.g. email@blossompot.com)."
      );
    }
...
async function sendQueuedEmail(
  ...
  await sendViaSes({
    ...
    html,
    text: htmlToText(html),
    fromName: (campaign.senderName || settings.defaultSenderName || "BlossomPot").trim(),
    fromEmail: (
      campaign.senderEmail ||
      settings.defaultSenderEmail ||
      "email@blossompot.com"
