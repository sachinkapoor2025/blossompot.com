...
async function loadSesSender(): Promise<{
  ...
  return {
    fromName: settings.defaultSenderName || "BlossomPot",
    fromEmail: settings.defaultSenderEmail || process.env.SES_FROM_EMAIL || "order@blossompot.com",
    replyTo: settings.defaultReplyTo || process.env.SES_REPLY_TO || "order@blossompot.com",
  };
}
...
export async function sendReminderEmailsHandler(event: APIGatewayProxyEventV2) {
  ...
  try {
    ...
    const siteUrl = process.env.SITE_URL || "https://www.blossompot.com";
