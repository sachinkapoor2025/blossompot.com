...
import { sesEmailKeys, sesSettingsSchema, type SesSettings } from "@blossompot/shared";
...
export function formatSesFromAddress(fromName: string, fromEmail: string): string {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new SesSendError(
      `Invalid From email address "${fromEmail || "(empty)"}". Use a verified sender (e.g. order@blossompot.com).`
    );
  }
...
export function formatSesError(err: unknown): SesSendError {
  ...
  if (/MailFromDomainNotVerified|EmailAddressNotVerified|not verified/i.test(combined)) {
    return new SesSendError(
      `SES identity not verified (${code}): ${raw}. Verify blossompot.com and ${process.env.SES_FROM_EMAIL || "order@blossompot.com"} in Amazon SES (${region}).`,
      { code, httpStatusCode, cause: err }
    );
  }
 ...
 * Transactional order mail uses apps/api/src/lib/email.ts (smtp.blossompot.com).
...
function envMarketingSmtp(): MarketingSmtp | null {
  ...
  const user = process.env.MARKETING_SMTP_USER?.trim() || "order@blossompot.com";
  if (/^(smtp|mail)\.blossompot\.com$/i.test(host)) return null;
...
async function seedMarketingSmtpPasswordIfNeeded(stored: Partial<SesSettings>): Promise<Partial<SesSettings>> {
  ...
  const seeded = sesSettingsSchema.parse({
    ...
    defaultSenderName: stored.defaultSenderName || "BlossomPot",
    defaultSenderEmail:
      stored.defaultSenderEmail ||
      process.env.MARKETING_FROM_EMAIL ||
      process.env.SES_FROM_EMAIL ||
      "email@blossompot.com",
    defaultReplyTo:
    ...
    smtpUser: stored.smtpUser?.trim() || process.env.MARKETING_SMTP_USER || "order@blossompot.com",
...
async function resolveTransport(): Promise<ResolvedTransport> {
  ...
  // Refuse transactional hosts for marketing — order mail uses smtp.blossompot.com separately.
  const isTransactionalHost = /^(smtp|mail)\.blossompot\.com$/i.test(host);
  ...
  if (mode === "smtp" && isTransactionalHost) {
    value = {
      mode: "misconfigured",
      message:
        "Marketing campaigns must use Mailercloud (smtp-prod.mailrcld.com), not transactional SMTP (smtp.blossompot.com). Update Admin → Email → Settings.",
    };
...
async function sendViaMarketingSmtp(
  ...
  // (e.g. email@blossompot.com), not necessarily the SMTP login user.
  const fromEmail = (input.fromEmail || smtp.user).trim();
  const fromName = (input.fromName || "BlossomPot").trim();
  ...
  try {
    ...
    if (/invalid sender|no valid From address/i.test(raw)) {
      throw new SesSendError(
        `Marketing SMTP failed: Mailercloud rejected sender ${fromEmail}. Use a verified Sender ID (e.g. email@blossompot.com) as Default sender email in Admin → Email → Settings. SMTP login user can stay as provided by Mailercloud. Host: ${smtp.host}`,
