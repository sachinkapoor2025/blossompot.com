...
import type { Order, Product, CartItem } from "@blossompot/shared";
import type { LeadCaptureInput } from "@blossompot/shared";
import {
  ORDER_STATUS,
...
function smtpHosts(): string[] {
  ...
  const extras = (process.env.SMTP_HOSTS ?? "mail.blossompot.com,smtp.blossompot.com")
    .split(",")
    .map((h) => h.trim())
    .filter(Boolean);
...
export async function sendNewsletterEmails(input: {
  ...
  const customer = await sendEmail({
    ...
    text: `You spun the Discount of the Day wheel at BlossomPot!
...
Admin: https://www.blossompot.com/admin/products`
...
We deliver to all 50 US states in 5–7 business days after dispatch.${footer}`,
    ...
    case ORDER_STATUS.ACCEPTED:
      return {
        subject: `Your Order is Confirmed! — ${shortId} | ${SITE_NAME}`,
...
If you need help, reply to this email and our team will assist.${footer}`,
    ...
    case ORDER_STATUS.DELIVERED:
      return {
        subject: `Your Order Has Been Delivered! — ${shortId} | ${SITE_NAME}`,
    ...
    case ORDER_STATUS.COMPLETE:
      return {
        subject: `Your Order is Complete! — ${shortId} | ${SITE_NAME}`,
...
/** Internal inbox copy when fulfillment status changes (order@blossompot + NOTIFY_EMAIL list). */
 ...
 * Sends to the customer AND order@blossompot / NOTIFY_EMAIL so the team sees updates
 ...
 * Duplicate saves of the same status are skipped via previousNotificationStatus.
 * Missing customer email or WhatsApp number skips that channel without throwing.
 */
export async function notifyCustomerOrderStatusChange(
  ...
  try {
    ...
    if (
      opts?.previousNotificationStatus &&
      opts.previousNotificationStatus === order.status
    ) {
    ...
    const customerEmail = order.shippingAddress?.email?.trim();
    let emailResult: EmailSendResult = { ok: true, skipped: true };

    if (!smtpConfigured()) {
      emailResult = { ok: false, skipped: true, error: "SMTP not configured" };
    } else if (!customerEmail?.includes("@")) {
      emailResult = { ok: true, skipped: true, error: "No customer email" };
    } else {
    ...
    const whatsappMessage =
      order.status === ORDER_STATUS.ACCEPTED
        ...
        : order.status === ORDER_STATUS.DELIVERED
          ? buildOrderDeliveredWhatsAppMessage(order, "delivered")
          : order.status === ORDER_STATUS.COMPLETE
            ? buildOrderDeliveredWhatsAppMessage(order, "complete")
            : orderStatusWhatsAppMessage({
                name: order.shippingAddress?.name?.split(" ")[0],
                orderId: order.orderId,
                status: order.status,
...
${
  input.confirmedSale || extreme
    ? "Thank you for confirming your BlossomPot order. Here is your reserved discount:"
    : "Thank you for considering BlossomPot. We've reserved a personal discount for you:"
}
...
${siteUrl()}`;
  ...
  // Same inbox list as order/contact alerts (order@blossompot.com + team) — not marketing SMTP.
...
— ${SITE_NAME} Admin`;
  ...
  const customer = input.customerEmail
    ? await sendEmail({
        ...
        subject: `${saleTag}Your ${input.discountPercent}% BlossomPot coupon (${input.code}) — valid ${hoursLabel}`,
        text: customerText,
        replyTo: notifyAddress(),
      })
