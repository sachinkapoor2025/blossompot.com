...
export async function seedIfEmpty() {
  ...
  if (!catalog) {
    console.log("No blossompot-catalog.json found — skipping seed.");
    return;
  }

  const timestamp = now();
  console.log(`Seeding in-memory DB: ${catalog.products.length} BlossomPot products...`);
  ...
  console.log("BlossomPot demo data ready.");
}
