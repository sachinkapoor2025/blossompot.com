...
describe("formatSesFromAddress", () => {
  it("formats a simple display name", () => {
    assert.equal(formatSesFromAddress("BlossomPot", "order@blossompot.com"), "BlossomPot <order@blossompot.com>");
  });

  it("returns bare email when name is empty", () => {
    assert.equal(formatSesFromAddress("  ", "order@blossompot.com"), "order@blossompot.com");
  });

  it("quotes names with special characters", () => {
    assert.equal(
      formatSesFromAddress('Usa "Rakhi"', "order@blossompot.com"),
      `"Usa \\"Rakhi\\"" <order@blossompot.com>`
    );
  });

  it("rejects invalid from email", () => {
    assert.throws(() => formatSesFromAddress("BlossomPot", ""), /Invalid From email/);
  });
});
