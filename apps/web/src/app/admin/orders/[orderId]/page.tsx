...
import type { Order, RateQuote } from "@blossompot/shared";
import {
  ORDER_STATUS,
...
export default function AdminOrderDetailPage() {
  ...
  const syncVendorTrackingState = (o: AdminOrder) => {
    ...
    // Keep legacy single fields in sync with BlossomPot lane (or sole vendor).
    const us = rows.find((r) => r.vendorSlug === VENDOR_BLOSSOMPOT) ?? rows[0];
            ...
            <div className="mt-3 space-y-2 border-t border-slate-100 pt-3">
              ...
              {multiVendor && (
                <p className="text-[11px] text-slate-500">
                  Mixed cart: Orange County and BlossomPot each get their own AWB. Order becomes Shipped
          ...
          <section className="bg-white border rounded-xl p-5">
            ...
            <form onSubmit={handleUpdate} className="space-y-3">
              ...
              {isAcceptOnly && !isReviveFromCancelled && (
                <p className="text-xs text-slate-500">
                  Confirming the order starts fulfillment. Add tracking when you move it to Processing or Shipped.
                </p>
              )}
