...
import { contactFormSchema } from "@blossompot/shared";
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: data.error ?? "Could not send message. Please try WhatsApp or email order@blossompot.com." },
    ...
    if (data.emailSent === false) {
      return NextResponse.json(
        { error: "Your message was saved but email could not be sent. Please email order@blossompot.com directly." },
        { status: 502 }
      );
    }
