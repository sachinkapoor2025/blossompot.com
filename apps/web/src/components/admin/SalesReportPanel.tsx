...
function downloadSalesExcel(period: SalesPeriodReport) {
  downloadCsv(
    `blossompot-sales-${period.period}-${new Date().toISOString().slice(0, 10)}.csv`,
    [
      ["BlossomPot Sales Report"],
      ["Period", period.label],
      ["From", period.from],
      ["To", period.to],
...
function SalesReportPrintView({ period }: { period: SalesPeriodReport }) {
  return (
    <div className="p-8 text-sm text-black bg-white">
      <p className="text-2xl font-bold mb-1">BlossomPot — Sales Report</p>
