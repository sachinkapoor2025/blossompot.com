...
export function FlowerGuideNav({ current }: { current?: string }) {
  return (
    <nav aria-label="Flower Guide" className="border-b border-line bg-surface/80 backdrop-blur-sm">
          ...
          <li>
            <Link
              ...
              className={`whitespace-nowrap rounded-full px-3 py-1.5 font-medium ${
                current === "/flower-guide" ? "bg-primary text-white" : "text-muted hover:bg-petal hover:text-primary"
              }`}
            >
              Overview
            </Link>
              ...
              <Link
                ...
                className={`whitespace-nowrap rounded-full px-3 py-1.5 font-medium ${
                  current === item.href || (current && current.startsWith(`${item.href}/`))
                    ...
                    : "text-muted hover:bg-petal hover:text-primary"
                }`}
              >
                {item.label}
              </Link>
