...
export function orderedCountryDialCodes(): CountryDialCode[] {
  ...
  const pinned = PINNED.map((iso) => byIso.get(iso)).filter(Boolean) as CountryDialCode[];
  const rest = ALL.filter((c) => !PINNED.includes(c.iso as (typeof PINNED)[number])).sort((a, b) =>
