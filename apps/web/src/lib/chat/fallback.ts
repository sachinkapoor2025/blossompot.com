...
const OFF_TOPIC_REPLY = `I'm here specifically to help with BlossomPot — sending Rakhi to the USA, our products, shipping, and orders. Is there something about Rakhi delivery I can help with?

Browse our catalog: [All Products](${siteUrl}/products) · WhatsApp: ${site.whatsappDisplay}`;

const SITE_KEYWORDS =
  /\b(rakhi|raksha|bandhan|usa|us\b|shipping|deliver|order|payment|stripe|razorpay|product|categor|bhaiya|bhabhi|lumba|kids|combo|chocolate|roli|chawal|california|texas|new york|florida|india|checkout|cart|price|track|support|blossompot)\b/i;
...
export function fallbackChatReply(userMessage: string): string {
  ...
  return `Thanks for your question! BlossomPot delivers premium Rakhis to all 50 US states in 5–7 business days. Sisters order from India, UK, Canada & worldwide.\n\n- [Shop all Rakhis](${siteUrl}/products)\n- [Shipping info](${siteUrl}/shipping)\n- [FAQ](${siteUrl}/faq)\n\nFor order help: WhatsApp ${site.whatsappDisplay} or ${site.supportEmail}`;
}
