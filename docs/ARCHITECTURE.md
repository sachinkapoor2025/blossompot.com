...
Per-domain tables (`PAY_PER_REQUEST`), named `blossompot-<domain>-{env}` and wired into
...
**USPS tracking sync (single source of truth):** Order `status` plus carrier fields (`carrierTrackingStatus`, `carrierStatusDetail`, `trackingEvents[]`, `lastTrackingSyncAt`, `lastTrackingSyncError`, `lastTrackingNotificationStatus`, `deliveredAt`, `estimatedDeliveryAt`) live on the order item. Admin portal and customer My Orders / order detail both read this same record — there is no separate customer status cache. Cron `processUspsTrackingSync` (EventBridge every 15 minutes on `ReviewEmailsCronFunction`, task `trackingSync`) queries GSI3 for active poll statuses (`processing`/`shipped`/`in_transit`/`out_for_delivery`/`delivery_exception` with a tracking number), calls USPS Tracking API v3, maps via `@blossompot/shared` `mapCarrierTrackingPhase`, advances `status` only forward (never overwrites `cancelled`/`refunded`/`complete`), emails only on meaningful status change (`lastTrackingNotificationStatus` dedupe), and stops polling once status is `delivered`/`complete`. Admin manual refresh: `POST /admin/orders/{orderId}/tracking/sync`. Customer read: `GET /orders/{orderId}/tracking` (or full `GET /orders/{orderId}`).
...
**WhatsApp (optional, additive):** When Meta Cloud API (`WHATSAPP_TOKEN` + `WHATSAPP_PHONE_NUMBER_ID`) and/or Twilio (`TWILIO_*`) env vars are set, the same customer notifications also send via WhatsApp (`apps/api/src/lib/whatsapp.ts` → `notifyCustomerWhatsApp`): welcome/admin/abandoned coupons, order paid + status updates, pending-payment reminders, review requests, contact ack, abandoned-cart recovery, **and gifting occasion reminders when the customer opted in**. Email remains primary — WhatsApp failures never block checkout or SMTP. Admin abandoned coupons still return a `wa.me` deep link when APIs are unset. For Meta out-of-session sends, set an approved `WHATSAPP_TEMPLATE_NAME` (freeform text only works inside the 24h customer-care window). Admin → Gifting Assistant can toggle `whatsappEnabled`; sends stay skipped if APIs are unset.

**Personal gifting assistant:** Storefront `/remember`, account dashboard (My People, Blossom Calendar, gift history, membership), `/forgot-occasion`, and `/gifting/choose/{token}`. Membership signup is a 5-step journey (plan → events → reminder channel → review → payment) at `/account?tab=membership`. Event eligibility is computed from membership start date + duration (`packages/shared` catalog: Valentine's, Chocolate Day, birthdays, anniversaries, festivals, etc.); 3/6-month plans only show occasions inside the window. Data lives on the customers table (`USER#` / `RECIPIENT#` / `OCCASION#` / `SUBSCRIPTION` / `GIFTHISTORY#` / `LOYALTY`) plus config-table plans, settings (`CONFIG#GIFTING`), reminder queues, and notification logs. Cron `processGiftingReminders` sends 10/7/3-day reminders (admin-configurable) and a choice window (default 2 hours). After no response the system **recommends** a scored gift; it does **not** auto-charge. APIs: `/gifting/*` and `/admin/gifting*`. Admin **Membership / Reminder Orders** (`/admin/membership-orders`) lists paid and pending memberships with selected events, dates, and WhatsApp/Email preferences — `GET /admin/gifting/subscriptions`. Event preview: `GET /gifting/membership/events`.
...
**Pending-payment unsubscribe:** Reminder emails include a link to `/unsubscribe/payment-reminders` (email prefilled). `POST /pending-payment-unsubscribe` stores the address in DynamoDB table `blossompot-pending-payment-unsub-{env}` (`PENDING_PAYMENT_UNSUB_TABLE`). The cron skips any order whose `shippingAddress.email` is on that list. This list is separate from SES marketing suppression.
...
| PATCH | `/admin/orders/{orderId}` | Admin: update status + tracking; emails customer + order@blossompot on each status step; schedules review email 1 day after delivered |
