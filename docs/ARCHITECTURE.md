...
**Personal gifting assistant:** Storefront `/remember`, account dashboard (My People, Blossom Calendar, gift history, membership), `/forgot-occasion`, and `/gifting/choose/{token}`. Data lives on the customers table (`USER#` / `RECIPIENT#` / `OCCASION#` / `SUBSCRIPTION` / `GIFTHISTORY#` / `LOYALTY`) plus config-table plans, settings (`CONFIG#GIFTING`), reminder queues, and notification logs. Cron `processGiftingReminders` sends 10/7/3-day reminders (admin-configurable) and a choice window (default 2 hours). After no response the system **recommends** a scored gift; it does **not** auto-charge. APIs: `/gifting/*` and `/admin/gifting*`.
