...
and `CONFIG_TABLE` (each defaulting to `blossompot-<domain>-<ENVIRONMENT>`). Create the
local tables with `npm run db:setup` (or set `USE_MEMORY_DB=true`).

Migrating from the legacy single table? Run:

```bash
ENVIRONMENT=prod LEGACY_TABLE=blossompot-prod npm run migrate:multitable
```
...
SAM build runs from the monorepo root so the local `@blossompot/shared` workspace package resolves during `sam build`.
