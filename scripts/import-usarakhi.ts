/**
 * Import categories + products from blossompot.com (WooCommerce Store API)
 * into DynamoDB. Also writes scripts/data/blossompot-catalog.json for local seed.
 *
 * Usage:
 *   npm run import:blossompot -- --fetch-only           # refresh catalog JSON
 *   ENVIRONMENT=prod npm run import:blossompot          # import to AWS (prod tables)
 */
...
const CATALOG_PATH = join(process.cwd(), "scripts/data/blossompot-catalog.json");
const WC_BASE = "https://blossompot.com/wp-json/wc/store/v1";
...
async function fetchGalleryImages(slug: string): Promise<string[]> {
  try {
    const res = await fetch(`https://blossompot.com/product/${encodeURIComponent(slug)}/`, {
      headers: { "User-Agent": "BlossomPot-Catalog-Import/1.0" },
    });
    if (!res.ok) return [];
...
async function main() {
  ...
  if (!refresh && existsSync(CATALOG_PATH)) {
    ...
    console.log("Fetching catalog from blossompot.com...");
    catalog = await fetchCatalog();
    mkdirSync(join(process.cwd(), "scripts/data"), { recursive: true });
    writeFileSync(CATALOG_PATH, JSON.stringify(catalog, null, 2));
    const apiCatalogPath = join(process.cwd(), "apps/api/src/data/blossompot-catalog.json");
