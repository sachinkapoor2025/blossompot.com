...
import { metaDescription, productKeys } from "@blossompot/shared";
...
function buildDescription(name: string, spec: Spec): string {
  ...
  return [
    `<p><strong>${esc(name)}</strong> is a premium designer Rakhi choice for USA delivery. Sisters in India, the UK, Canada, Australia, and worldwide order from BlossomPot so brothers across America receive festive packaging with reliable domestic shipping for Raksha Bandhan.</p>`,
    ...
    `<p><strong>Why sisters choose BlossomPot for USA delivery:</strong></p>`,
...
function buildSeoDescription(name: string, spec: Spec): string {
  ...
  return metaDescription(
    `Send ${name} to USA — ${qty}${ritual}. Domestic shipping for Raksha Bandhan from BlossomPot.`
  );
}
...
async function main() {
  ...
  const TABLE = process.env.PRODUCTS_TABLE ?? `blossompot-products-${ENV}`;
