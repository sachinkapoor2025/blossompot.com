...
const host = process.env.SMTP_HOST ?? "smtp.blossompot.com";
const user = process.env.SMTP_USER ?? "order@blossompot.com";
...
async function trySend(port: number, secure: boolean) {
  ...
  await transporter.sendMail({
    from: `"BlossomPot Test" <${user}>`,
    to: testTo,
    subject: `[BlossomPot] SMTP test ${new Date().toISOString()}`,
...
async function main() {
  const hosts = [host, "mail.blossompot.com"].filter((h, i, a) => a.indexOf(h) === i);
