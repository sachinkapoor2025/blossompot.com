# Load testing — BlossomPot (~1000 concurrent shoppers)
...
| Staging | SAM stack `blossompot-staging` | Prefer this for 1000 VU runs |
...
- Lambda `blossompot-api-staging` (or `-prod`): `ConcurrentExecutions`, `Duration`, `Errors`, `Throttles`
- DynamoDB tables `blossompot-*-staging`: `ConsumedReadCapacityUnits`, `ConsumedWriteCapacityUnits`, `ThrottledRequests`
...
aws cloudwatch get-metric-statistics --namespace AWS/Lambda \
  --metric-name ConcurrentExecutions --dimensions Name=FunctionName,Value=blossompot-api-staging \
