 ...
 * table (blossompot-<env>) into the new per-domain tables.
 *
 * Products/categories are re-seeded via `npm run import:blossompot`, and carts are
 * ephemeral, so they are intentionally NOT migrated here.
 *
 * Usage (against AWS):
 *   ENVIRONMENT=prod LEGACY_TABLE=blossompot-prod npx tsx scripts/migrate-to-multitable.ts
 *
 * Safe to re-run (idempotent puts). The legacy table is only read, never deleted.
 */
import { DynamoDBClient } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, ScanCommand, PutCommand } from "@aws-sdk/lib-dynamodb";
import { orderKeys, customerKeys, ORDER_STATUS } from "@blossompot/shared";

const ENV = process.env.ENVIRONMENT ?? "dev";
const LEGACY_TABLE = process.env.LEGACY_TABLE ?? `blossompot-${ENV}`;
const ORDERS_TABLE = process.env.ORDERS_TABLE ?? `blossompot-orders-${ENV}`;
const CUSTOMERS_TABLE = process.env.CUSTOMERS_TABLE ?? `blossompot-customers-${ENV}`;
...
async function main() {
  ...
  console.log("Legacy table left intact. Products/categories: run `npm run import:blossompot`.");
}
