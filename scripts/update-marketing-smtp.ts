 ...
 *   MARKETING_SMTP_USER  (default order@blossompot.com)
...
const TABLE = process.env.EMAIL_CAMPAIGNS_TABLE ?? `blossompot-email-campaigns-${ENV}`;
...
async function main() {
  ...
  const merged = {
    ...
    smtpSecure,
    smtpUser:
      process.env.MARKETING_SMTP_USER?.trim() || prev.smtpUser || "order@blossompot.com",
    smtpPassword: pass,
  };
